...
namespace BfmeWorkshopKit.Logic
{
    public static class BfmeWorkshopAuthManager
    {
        public static async Task<BfmeWorkshopAuthInfo> Authenticate(string email, string password)
        {
            ...
            string requestResponse = await ApiRequestManagger.Get(BfmeWorkshopAuthInfo.Unauthenticated, "auth/login", new Dictionary<string, string>()
            {
                { "email", email },
                { "password", passwordHash },
            });

            string[] authResult = JsonConvert.DeserializeObject<string[]>(requestResponse) ?? new string[3];
